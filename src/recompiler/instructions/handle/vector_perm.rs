...
pub(crate) fn handle_vmrg_hilo_common(ctx: &mut LowerCtx, kind: &str, hi: bool) -> bool {
    ...
    let d = ctx.op_reg(0);
    let a = ctx.op_reg(2);
    let b = ctx.op_reg(1); // keep your operand swap
    ...
    match kind {
        "epi8" => {
            ...
            ctx.println_fmt(format_args!(
                "\t\t\t{vd}.u8[2*i]     = {va}.u8[{start} + i];"
            ));
            ctx.println_fmt(format_args!(
                "\t\t\t{vd}.u8[2*i + 1] = {vb}.u8[{start} + i];"
            ));
            ctx.println("\t\t}");
            ctx.println("\t}");
        }
        "epi16" => {
            ...
            ctx.println_fmt(format_args!(
                "\t\t\t{vd}.u16[2*i]     = {va}.u16[{start} + i];"
            ));
            ctx.println_fmt(format_args!(
                "\t\t\t{vd}.u16[2*i + 1] = {vb}.u16[{start} + i];"
            ));
            ctx.println("\t\t}");
            ctx.println("\t}");
        }
        _ => {
            ...
            ctx.println_fmt(format_args!(
                "\t\t\t{vd}.u32[2*i]     = {va}.u32[{start} + i];"
            ));
            ctx.println_fmt(format_args!(
                "\t\t\t{vd}.u32[2*i + 1] = {vb}.u32[{start} + i];"
            ));
            ctx.println("\t\t}");
            ctx.println("\t}");
        }
...
pub(crate) fn handle_vperm_like(ctx: &mut LowerCtx) -> bool {
    ...
    ctx.println("\t\tfor i in 0..16 {");
    ctx.println_fmt(format_args!(
        "\t\t\tlet sel = ({vc}.u8[i] & 0x1F) as usize;"
    ));
    ctx.println_fmt(format_args!(
        "\t\t\t{vd}.u8[i] = if sel < 16 {{ {va}.u8[sel] }} else {{ {vb}.u8[sel - 16] }};"
    ));
    ctx.println("\t\t}");
    ctx.println("\t}");
    true
...
pub(crate) fn handle_vsel(ctx: &mut LowerCtx) -> bool {
    ...
    ctx.println_fmt(format_args!(
        "\t\t\t{vd}.u8[i] = ({va}.u8[i] & mask) | ({vb}.u8[i] & !mask);"
    ));
    ctx.println("\t\t}");
    ctx.println("\t}");
    true
...
pub(crate) fn handle_vrlimi128(ctx: &mut LowerCtx) -> bool {
    ...
    ctx.println_fmt(format_args!(
        "\t// build permuted temp from {va}.f32 using sel={sel}"
    ));
    ctx.println(
        "\t// sel=0=>[0,1,2,3], sel=1=>[3,0,1,2], sel=2=>[2,3,0,1], sel=3=>[1,2,3,0]",
    );

    // Declare perm and fill it in an unsafe block for each sel
    ctx.println("\tlet mut perm: [f32; 4];");
    match sel {
        0 => {
            // identity
            ctx.println_fmt(format_args!(
                "\tunsafe {{ perm = [{va}.f32[0], {va}.f32[1], {va}.f32[2], {va}.f32[3]]; }}"
            ));
        }
        1 => {
            // [3,0,1,2]
            ctx.println_fmt(format_args!(
                "\tunsafe {{ perm = [{va}.f32[3], {va}.f32[0], {va}.f32[1], {va}.f32[2]]; }}"
            ));
        }
        2 => {
            // [2,3,0,1]
            ctx.println_fmt(format_args!(
                "\tunsafe {{ perm = [{va}.f32[2], {va}.f32[3], {va}.f32[0], {va}.f32[1]]; }}"
            ));
        }
        _ => {
            // 3 => [1,2,3,0]
            ctx.println_fmt(format_args!(
                "\tunsafe {{ perm = [{va}.f32[1], {va}.f32[2], {va}.f32[3], {va}.f32[0]]; }}"
            ));
        }
    }
    ...
    ctx.println("\t\tfor i in 0..4 {");
    ctx.println_fmt(format_args!(
        "\t\t\tlet take = ((({blend_mask}u32) >> i) & 1) != 0;"
    ));
    ctx.println_fmt(format_args!(
        "\t\t\tif take {{ {vd}.f32[i] = perm[i]; }}"
    ));
    ctx.println("\t\t}");
    ctx.println("\t}");
    true
