...
pub(crate) fn handle_and(ctx: &mut LowerCtx) -> bool {
    ...
    ctx.println_fmt(format_args!("\tunsafe {{ {rd}.u64 = {ra}.u64 & {rb}.u64; }}"));
...
pub(crate) fn handle_andis(ctx: &mut LowerCtx) -> bool {
    ...
    let d = ctx.op_reg(0);
    let a = ctx.op_reg(1);
    let imm = ((ctx.op_imm(2) as u32) << 16) as u64;
...
pub(crate) fn handle_oris(ctx: &mut LowerCtx) -> bool {
    ...
    let d = ctx.op_reg(0);
    let a = ctx.op_reg(1);
    let imm = (ctx.op_imm(2) as u64) << 16;
...
pub(crate) fn handle_not(ctx: &mut LowerCtx) -> bool {
    ...
    // not rD,rS  => rD = ~rS
    ctx.println_fmt(format_args!("\tunsafe {{ {rd}.u64 = !{rs}.u64; }}"));
    if ctx.insn.mnemonic().unwrap_or_default().ends_with('.') {
        let cr0 = ctx.cr(0).to_string();
        let xer = ctx.xer().to_string();
        ctx.println_fmt(format_args!(
