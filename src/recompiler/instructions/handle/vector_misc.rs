    ...
    match mode {
        "neg" => {
            ...
            ctx.println_fmt(format_args!(
                "\t\tunsafe {{ {vd}.f32[i] = {va}.f32[i].floor(); }}",
            ));
            ctx.println("\t}");
        }
        "nearest" => {
            // VRFIN: nearest (ties to even in Rust's round)
            ctx.println("\tfor i in 0..4 {");
            ctx.println_fmt(format_args!(
                "\t\tunsafe {{ {vd}.f32[i] = {va}.f32[i].round(); }}",
            ));
            ctx.println("\t}");
        }
        _ => {
            // default: toward zero (VRFIZ)
            ctx.println("\tfor i in 0..4 {");
            ctx.println_fmt(format_args!(
                "\t\tunsafe {{ {vd}.f32[i] = {va}.f32[i].trunc(); }}",
            ));
            ctx.println("\t}");
        }
    }
